# Fundamentus Fii Printed CLI Agent Guide

This directory is a generated `fundamentus-fii-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
fundamentus-fii-pp-cli doctor --json
fundamentus-fii-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
fundamentus-fii-pp-cli which "<capability>" --json
fundamentus-fii-pp-cli <command> --help
```
...
```bash
fundamentus-fii-pp-cli <command> --agent
```
...
```bash
fundamentus-fii-pp-cli <command> --help
fundamentus-fii-pp-cli <command> --dry-run --agent
```
