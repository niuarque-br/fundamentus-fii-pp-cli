...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "fundamentus-fii-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FUNDAMENTUS_FII_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FUNDAMENTUS_FII_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "fundamentus-fii-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/fundamentus-fii-pp-cli/feedback.jsonl.
When ` + "`FUNDAMENTUS_FII_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FUNDAMENTUS_FII_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  fundamentus-fii-pp-cli feedback list
  fundamentus-fii-pp-cli feedback list --limit 5
  fundamentus-fii-pp-cli feedback list --json`,
